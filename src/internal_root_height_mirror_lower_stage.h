#ifndef TINYDB_INTERNAL_ROOT_HEIGHT_MIRROR_LOWER_STAGE_H
#define TINYDB_INTERNAL_ROOT_HEIGHT_MIRROR_LOWER_STAGE_H

#include "internal_nonroot_merge_stage.h"

/*
 * Stage only the lower merge needed by the mirrored height-4 contraction:
 *
 *   obsolete [A, removed] + kept [C, D] -> kept [A, C, D]
 *
 * Both parents must be minimum two-child non-root internals under left_grand.
 * The rebuilt kept parent is prepared to move under new_parent_page_num. Source
 * images for obsolete and removed remain immutable. Caller-visible mutation is
 * copied back only after the complete staged lower topology validates.
 */
static inline bool tinydb_stage_root_height_mirror_lower_merge(
    ...
    if (!tinydb_nonroot_merge_leaf_valid(removed, removed_capacity,
        ...
        maxes[0] >= removed_key || removed_key >= mins[1]) {
        return false;
    }
    ...
    if (!tinydb_stage_leaf_sibling_relink(leaf_scratch[0], PAGE_SIZE, true,
                                          removed_leaf_page_num,
                                          survivor_leaf_page_nums[1]) ||
        !tinydb_stage_leaf_sibling_relink(leaf_scratch[1], PAGE_SIZE, false,
                                          removed_leaf_page_num,
...
#endif /* TINYDB_INTERNAL_ROOT_HEIGHT_MIRROR_LOWER_STAGE_H */
