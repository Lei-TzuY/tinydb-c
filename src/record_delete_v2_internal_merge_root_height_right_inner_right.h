#ifndef TINYDB_RECORD_DELETE_V2_INTERNAL_MERGE_ROOT_HEIGHT_RIGHT_INNER_RIGHT_H
#define TINYDB_RECORD_DELETE_V2_INTERNAL_MERGE_ROOT_HEIGHT_RIGHT_INNER_RIGHT_H
...
/*
 * Height-4 -> height-3 contraction for the right child of the left minimum
 * bottom parent under the right grandparent:
 *
 *   p2 [A, removed] + p3 [C, D] -> p3 [A, C, D]
 *
 * The proven mirror lower-stage repairs A<->C. The two bottom parents from the
 * left grandparent are then staged under the right grandparent, which becomes
 * [p0,p1,p3] and is promoted into the stable catalog root. No live page is
 * exposed until all staged images validate.
 */
static inline TinyDBInternalMergeRootResult
 tinydb_try_delete_v2_internal_merge_root_height_right_inner_right(
    Table* table,
    ...
    if (!tinydb_parent_stage_validate(right_grand, PAGE_SIZE) ||
        ...
        tinydb_parent_stage_key_at(right_grand, 0u) != removed_key) {
        return TINYDB_INTERNAL_MERGE_ROOT_NOT_APPLICABLE;
    }
    ...
    uint32_t first_survivor_count = 0u;
    uint32_t first_survivor_max = 0u;
    if (!tinydb_leaf_page_count(survivor_after[0], PAGE_SIZE,
                                &first_survivor_count) ||
        first_survivor_count == 0u ||
        !tinydb_leaf_page_key_at(survivor_after[0], PAGE_SIZE,
                                 first_survivor_count - 1u,
                                 &first_survivor_max) ||
        left0_max >= left_subtree_max ||
        left_subtree_max >= first_survivor_max ||
        first_survivor_max >= removed_key || removed_key >= kept_max) {
        return TINYDB_INTERNAL_MERGE_ROOT_NOT_APPLICABLE;
    }
    ...
    if (!tinydb_v2_publish_batch(entries, 7u, TINYDB_V2_PUBLISH_NO_FAIL)) {
        tinydb_internal_merge_root_message(
            message, message_size,
            "right-inner-right recursive V2 root-height publication failed");
        return TINYDB_INTERNAL_MERGE_ROOT_FAILURE;
    }
...
#endif /* TINYDB_RECORD_DELETE_V2_INTERNAL_MERGE_ROOT_HEIGHT_RIGHT_INNER_RIGHT_H */
