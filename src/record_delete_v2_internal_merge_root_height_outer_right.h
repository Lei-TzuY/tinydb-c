#ifndef TINYDB_RECORD_DELETE_V2_INTERNAL_MERGE_ROOT_HEIGHT_OUTER_RIGHT_H
#define TINYDB_RECORD_DELETE_V2_INTERNAL_MERGE_ROOT_HEIGHT_OUTER_RIGHT_H
...
/*
 * Publish the outer-right height-4 -> height-3 contraction.
 *
 * The singleton removed leaf is the rightmost leaf of the whole tree and the
 * right child of a minimum bottom parent. The lower merge keeps the left bottom
 * parent as [A,B,C]. Both minimum grandparents are then eliminated: the two
 * bottom parents formerly under the left grandparent are staged under the right
 * grandparent, which is rebuilt as [p0,p1,kept] and promoted into the stable
 * root. All mutations remain scratch-only until one validated batch publish.
 */
static inline TinyDBInternalMergeRootResult
 tinydb_try_delete_v2_internal_merge_root_height_outer_right(
    Table* table,
    ...
    if (!tinydb_parent_stage_validate(right_grand, PAGE_SIZE) ||
        ...
        tinydb_parent_stage_child_at(right_grand, 1u) != obsolete_num) {
        return TINYDB_INTERNAL_MERGE_ROOT_NOT_APPLICABLE;
    }

    uint32_t kept_num = tinydb_parent_stage_child_at(right_grand, 0u);
    ...
    uint32_t kept_old_max_count = 0u;
    uint32_t kept_old_max = 0u;
    if (!tinydb_leaf_page_count(survivor_after[1], PAGE_SIZE,
                                &kept_old_max_count) ||
        kept_old_max_count == 0u ||
        !tinydb_leaf_page_key_at(survivor_after[1], PAGE_SIZE,
                                 kept_old_max_count - 1u, &kept_old_max) ||
        tinydb_parent_stage_key_at(right_grand, 0u) != kept_old_max) {
        return TINYDB_INTERNAL_MERGE_ROOT_NOT_APPLICABLE;
    }
    ...
    uint32_t kept_max_count = 0u;
    uint32_t kept_max = 0u;
    if (!tinydb_leaf_page_count(survivor_after[2], PAGE_SIZE,
                                &kept_max_count) ||
        kept_max_count == 0u ||
        !tinydb_leaf_page_key_at(survivor_after[2], PAGE_SIZE,
                                 kept_max_count - 1u, &kept_max)) {
        return TINYDB_INTERNAL_MERGE_ROOT_NOT_APPLICABLE;
    }
    ...
    if (left0_max >= left_subtree_max || left_subtree_max >= kept_max) {
        return TINYDB_INTERNAL_MERGE_ROOT_NOT_APPLICABLE;
    }

    uint32_t left_last_num = tinydb_parent_stage_child_at(p_after[1], 1u);
    uint32_t right_first_num = survivor_nums[0];
    if (left_last_num == 0u || left_last_num == INVALID_PAGE_NUM ||
        left_last_num >= pager->num_pages) {
        return TINYDB_INTERNAL_MERGE_ROOT_NOT_APPLICABLE;
    }
    unsigned char left_last[PAGE_SIZE];
    memcpy(left_last, get_page(pager, left_last_num), PAGE_SIZE);
    uint32_t left_last_min = 0u, left_last_max = 0u;
    uint32_t left_last_prev = INVALID_PAGE_NUM, left_last_next = INVALID_PAGE_NUM;
    uint32_t right_first_min = 0u, right_first_max = 0u;
    uint32_t right_first_prev = INVALID_PAGE_NUM, right_first_next = INVALID_PAGE_NUM;
    if (!tinydb_nonroot_merge_leaf_valid(
            left_last, PAGE_SIZE, p_num[1],
            &left_last_min, &left_last_max, &left_last_prev, &left_last_next) ||
        !tinydb_nonroot_merge_leaf_valid(
            survivor_after[0], PAGE_SIZE, kept_num,
            &right_first_min, &right_first_max,
            &right_first_prev, &right_first_next) ||
        left_last_max != left_subtree_max ||
        left_last_next != right_first_num ||
        right_first_prev != left_last_num ||
        left_last_max >= right_first_min) {
        return TINYDB_INTERNAL_MERGE_ROOT_NOT_APPLICABLE;
    }
    (void)left_last_min;
    (void)left_last_prev;
    ...
    if (!tinydb_v2_publish_batch(entries, 7u, TINYDB_V2_PUBLISH_NO_FAIL)) {
        tinydb_internal_merge_root_message(
            message, message_size,
            "outer-right recursive slotted V2 root-height publication failed");
        return TINYDB_INTERNAL_MERGE_ROOT_FAILURE;
    }
...
#endif /* TINYDB_RECORD_DELETE_V2_INTERNAL_MERGE_ROOT_HEIGHT_OUTER_RIGHT_H */
