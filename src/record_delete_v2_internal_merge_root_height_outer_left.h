#ifndef TINYDB_RECORD_DELETE_V2_INTERNAL_MERGE_ROOT_HEIGHT_OUTER_LEFT_H
#define TINYDB_RECORD_DELETE_V2_INTERNAL_MERGE_ROOT_HEIGHT_OUTER_LEFT_H
...
/*
 * Publish the outer-left height-4 -> height-3 contraction.
 *
 * The singleton removed leaf is the leftmost leaf of the whole tree and the
 * left child of a minimum bottom parent. Its minimum sibling cannot lend, and
 * both grandparents under the one-key root are also minimum. The lower merge
 * keeps the right bottom parent as [B,C,D], then the two obsolete grandparents
 * are eliminated and the stable root adopts [kept,q0,q1] directly.
 */
static inline TinyDBInternalMergeRootResult
 tinydb_try_delete_v2_internal_merge_root_height_outer_left(
    Table* table,
    ...
    uint32_t left_survivor_count = 0u;
    uint32_t left_survivor_max = 0u;
    if (!tinydb_leaf_page_count(survivor_after[0], PAGE_SIZE,
                                &left_survivor_count) ||
        left_survivor_count == 0u ||
        !tinydb_leaf_page_key_at(survivor_after[0], PAGE_SIZE,
                                 left_survivor_count - 1u,
                                 &left_survivor_max) ||
        tinydb_parent_stage_key_at(left_grand, 0u) != left_survivor_max) {
        return TINYDB_INTERNAL_MERGE_ROOT_NOT_APPLICABLE;
    }
    ...
    uint32_t first_min = 0u, first_max = 0u;
    uint32_t first_prev = INVALID_PAGE_NUM, first_next = INVALID_PAGE_NUM;
    uint32_t last_min = 0u, last_max = 0u;
    uint32_t last_prev = INVALID_PAGE_NUM, last_next = INVALID_PAGE_NUM;
    uint32_t survivor_next = INVALID_PAGE_NUM;
    if (!tinydb_nonroot_merge_leaf_valid(
            right_first, PAGE_SIZE, q_num[0],
    ...
    if (!tinydb_v2_publish_batch(entries, 7u, TINYDB_V2_PUBLISH_NO_FAIL)) {
        tinydb_internal_merge_root_message(
            message, message_size,
            "outer-left recursive slotted V2 root-height publication failed");
        return TINYDB_INTERNAL_MERGE_ROOT_FAILURE;
    }
...
#endif /* TINYDB_RECORD_DELETE_V2_INTERNAL_MERGE_ROOT_HEIGHT_OUTER_LEFT_H */
