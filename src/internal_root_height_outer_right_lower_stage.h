#ifndef TINYDB_INTERNAL_ROOT_HEIGHT_OUTER_RIGHT_LOWER_STAGE_H
#define TINYDB_INTERNAL_ROOT_HEIGHT_OUTER_RIGHT_LOWER_STAGE_H

#include "internal_nonroot_merge_stage.h"

/*
 * Stage the outer-right lower merge needed by height-4 root contraction:
 *
 *   kept [A, B] + obsolete [C, removed] -> kept [A, B, C]
 *
 * The removed singleton is the rightmost leaf in the whole chain. The
 * operation repairs C.next to zero, reparents C to kept, preserves the already
 * reciprocal B<->C boundary, and rebuilds kept under new_parent_page_num.
 * Source images for obsolete and removed remain immutable; outputs are copied
 * only after the complete staged topology validates.
 */
static inline bool tinydb_stage_root_height_outer_right_lower_merge(
    ...
    if (kept_parent_page == NULL || obsolete_parent_page == NULL ||
        ...
        survivor_leaf_page_nums == NULL || kept_capacity < PAGE_SIZE ||
        obsolete_capacity < PAGE_SIZE || removed_capacity < PAGE_SIZE ||
    ...
    if (!tinydb_nonroot_merge_leaf_valid(removed, removed_capacity,
        ...
        removed_next != 0u || removed_prev != survivor_leaf_page_nums[2] ||
    ...
    if (!tinydb_stage_leaf_sibling_relink(leaf_scratch[2], PAGE_SIZE, true,
    ...
    if (!tinydb_leaf_page_next(leaf_scratch[2], PAGE_SIZE, &final_next) ||
        final_next != 0u) {
        return false;
    }
...
#endif /* TINYDB_INTERNAL_ROOT_HEIGHT_OUTER_RIGHT_LOWER_STAGE_H */
