#ifndef TINYDB_RECORD_DELETE_V2_INTERNAL_MERGE_BORROW_ROOT_H
#define TINYDB_RECORD_DELETE_V2_INTERNAL_MERGE_BORROW_ROOT_H
...
/*
 * Publish the first recursive V2 DELETE underflow repair.
 *
 * The supported live shape is the same deliberately narrow topology proven by
 * internal_merge_borrow_root_cascade_stage.h: a two-child left grandparent
 * would underflow after merging its two minimum leaf-parent children, so the
 * leftmost leaf-parent subtree is borrowed from a >=3-child right grandparent
 * under the stable root. The cascade terminates at the root by updating one
 * separator; tree height and root identity stay unchanged.
 */
static inline TinyDBInternalMergeRootResult
 tinydb_try_delete_v2_internal_merge_borrow_root(
    Table* table,
    ...
    if (!tinydb_parent_stage_validate(right_grand_after, PAGE_SIZE) ||
        ...
        tinydb_internal_borrow_read_u32(
            right_grand_after + PARENT_POINTER_OFFSET) != root_num ||
        tinydb_parent_stage_read_u32(
    ...
    if (donor_parent_num == 0u || donor_parent_num == INVALID_PAGE_NUM ||
        ...
        next_parent_num >= table->pager->num_pages ||
        donor_parent_num == next_parent_num ||
        tinydb_internal_borrow_page_free(table->pager, donor_parent_num) ||
        tinydb_internal_borrow_page_free(table->pager, next_parent_num)) {
        return TINYDB_INTERNAL_MERGE_ROOT_NOT_APPLICABLE;
    }
    ...
    memcpy(next_parent, get_page(table->pager, next_parent_num), PAGE_SIZE);
    ...
    if (next_left_num == 0u || next_left_num == INVALID_PAGE_NUM ||
        next_left_num >= table->pager->num_pages ||
        tinydb_internal_borrow_page_free(table->pager, next_left_num)) {
        return TINYDB_INTERNAL_MERGE_ROOT_NOT_APPLICABLE;
    }
    memcpy(next_left_before, get_page(table->pager, next_left_num), PAGE_SIZE);
    ...
    entries[3].page_num = kept_bottom_num;
    entries[3].target = (unsigned char*)get_page(table->pager, kept_bottom_num);
    entries[3].staged = kept_after;
    entries[4].page_num = donor_parent_num;
    entries[4].target = (unsigned char*)get_page(table->pager, donor_parent_num);
    ...
    if (!tinydb_v2_publish_batch(entries, 8u, TINYDB_V2_PUBLISH_NO_FAIL)) {
        tinydb_internal_merge_root_message(
            message, message_size,
            "recursive slotted V2 merge-borrow publication failed");
        return TINYDB_INTERNAL_MERGE_ROOT_FAILURE;
    }
...
#endif /* TINYDB_RECORD_DELETE_V2_INTERNAL_MERGE_BORROW_ROOT_H */
