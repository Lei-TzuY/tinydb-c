#ifndef TINYDB_INTERNAL_MERGE_BORROW_ROOT_CASCADE_STAGE_H
#define TINYDB_INTERNAL_MERGE_BORROW_ROOT_CASCADE_STAGE_H
...
/*
 * Stage the first genuine two-level V2 DELETE rebalance cascade.
 *
 * The supported shape is deliberately narrow and boundary-aligned:
 *
 *                  stable root
 *                 /           \
 *        left grandparent    right grandparent
 *           /       \          /      |      ...
 *       obsolete    kept      donor   next    ...
 *        /   \      /  \      /  \
 *       A     B  removed C    D    E
 *
 * - both bottom parents under the left grandparent are minimum two-child
 *   internals;
 * - removed is the singleton V2 *leftmost* leaf of the right/kept bottom
 *   parent;
 * - the lower merge keeps that right bottom parent as [A, B, C];
 * - this would leave the left grandparent with only one child, so the complete
 *   donor leaf-parent subtree [D, E] is borrowed from the left edge of the
 *   right grandparent;
 * - the repaired left grandparent becomes [kept, donor];
 * - the right grandparent must start with at least three children, so removing
 *   its non-rightmost donor leaves it valid and does not change its maximum;
 * - the root keeps the same two grandparent children and only raises its first
 *   separator from max(C) to max(E).
 *
 * All mutations occur in scratch images. The removed leaf and obsolete bottom
 * parent are source-only and may be reclaimed by a Pager/WAL caller only after
 * publishing the staged images. Donor leaves and the next subtree's leftmost
 * leaf are also source-only: their existing leaf-chain links prove that moving
 * the donor parent across the grandparent boundary does not require leaf-link
 * rewrites at that upper level.
 */
static inline bool tinydb_stage_internal_merge_borrow_from_right_grandparent(
    void* root_page,
    ...
    if (root_page == NULL || left_grand_page == NULL ||
        ...
        kept_bottom_parent_page == NULL || donor_parent_page == NULL ||
        removed_leaf_page == NULL || survivor_leaf_pages == NULL ||
        survivor_leaf_page_nums == NULL || donor_leaf_pages == NULL ||
        donor_leaf_page_nums == NULL ||
        next_subtree_leftmost_leaf_page == NULL ||
        root_capacity < PAGE_SIZE || left_grand_capacity < PAGE_SIZE ||
        right_grand_capacity < PAGE_SIZE ||
        obsolete_bottom_capacity < PAGE_SIZE || kept_bottom_capacity < PAGE_SIZE ||
        donor_parent_capacity < PAGE_SIZE || removed_leaf_capacity < PAGE_SIZE ||
        next_subtree_leftmost_capacity < PAGE_SIZE ||
        ...
        next_subtree_leftmost_leaf_page_num == 0u ||
        next_subtree_leftmost_leaf_page_num == INVALID_PAGE_NUM) {
        return false;
    }
    ...
    const unsigned char* next_left =
        (const unsigned char*)next_subtree_leftmost_leaf_page;
    ...
    if (!tinydb_parent_stage_validate(root, root_capacity) ||
        ...
        tinydb_parent_stage_read_u32(right_grand + PARENT_POINTER_OFFSET) !=
            root_page_num ||
        tinydb_parent_stage_read_u32(
            right_grand + INTERNAL_NODE_NUM_KEYS_OFFSET) < 2u ||
        tinydb_parent_stage_child_at(right_grand, 0u) != donor_parent_page_num ||
    ...
    uint32_t obsolete_max = tinydb_parent_stage_key_at(left_grand, 0u);
    ...
    if (survivor_max[0] != tinydb_parent_stage_key_at(obsolete, 0u) ||
        survivor_max[1] != obsolete_max || survivor_max[2] != old_left_max) {
        return false;
    }
    ...
    uint32_t donor_subtree_max = tinydb_parent_stage_key_at(right_grand, 0u);
    if (donor_max[0] != tinydb_parent_stage_key_at(donor, 0u) ||
        donor_max[1] != donor_subtree_max ||
        survivor_max[2] >= donor_min[0] ||
        survivor_next[2] != donor_leaf_page_nums[0] ||
        donor_prev[0] != survivor_leaf_page_nums[2] ||
    ...
    uint32_t next_parent_num = tinydb_parent_stage_child_at(right_grand, 1u);
    uint32_t next_min = 0u;
    uint32_t next_max = 0u;
    uint32_t next_prev = INVALID_PAGE_NUM;
    uint32_t next_next = INVALID_PAGE_NUM;
    if (next_subtree_leftmost_leaf_page_num == 0u ||
        next_subtree_leftmost_leaf_page_num == INVALID_PAGE_NUM ||
        !tinydb_nonroot_merge_leaf_valid(next_left,
                                         ...
                                         &next_prev,
                                         &next_next) ||
        donor_next[1] != next_subtree_leftmost_leaf_page_num ||
        next_prev != donor_leaf_page_nums[1] ||
        donor_subtree_max >= next_min) {
        return false;
    }
    (void)next_max;
    (void)next_next;
    ...
    bool donor_grand_max_changed = false;
    uint32_t donor_grand_new_max = 0u;
    if (!tinydb_stage_internal_child_remove(right_grand_scratch,
                                            PAGE_SIZE,
                                            donor_parent_page_num,
                                            donor_subtree_max,
                                            &removed_donor_index,
                                            &donor_grand_max_changed,
                                            &donor_grand_new_max) ||
        removed_donor_index != 0u || donor_grand_max_changed ||
        donor_grand_new_max != 0u) {
        return false;
    }
    ...
    if (!tinydb_parent_stage_validate(root_scratch, PAGE_SIZE) ||
        ...
        tinydb_parent_stage_key_at(root_scratch, 0u) != donor_subtree_max ||
        tinydb_parent_stage_key_at(root_scratch, 0u) <= old_left_max) {
        return false;
    }

    if (!tinydb_parent_stage_validate(left_grand_scratch, PAGE_SIZE) ||
        tinydb_parent_stage_child_at(left_grand_scratch, 0u) !=
            kept_bottom_parent_page_num ||
        tinydb_parent_stage_key_at(left_grand_scratch, 0u) != survivor_max[2] ||
        tinydb_parent_stage_child_at(left_grand_scratch, 1u) !=
            donor_parent_page_num ||
    ...
    if (!tinydb_leaf_page_next(survivor_scratch[2], PAGE_SIZE, &final_next) ||
        final_next != donor_leaf_page_nums[0]) {
        return false;
    }
...
#endif /* TINYDB_INTERNAL_MERGE_BORROW_ROOT_CASCADE_STAGE_H */
