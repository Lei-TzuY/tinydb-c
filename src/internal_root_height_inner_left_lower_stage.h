#ifndef TINYDB_INTERNAL_ROOT_HEIGHT_INNER_LEFT_LOWER_STAGE_H
#define TINYDB_INTERNAL_ROOT_HEIGHT_INNER_LEFT_LOWER_STAGE_H

#include "internal_nonroot_merge_stage.h"

/*
 * Stage the inner-left lower merge used by a height-4 root contraction:
 *
 *   prev + obsolete [removed, B] + kept [C, D]
 *       -> prev + kept [B, C, D]
 *
 * The removed singleton is not the chain head. The operation therefore repairs
 * both sides of the cross-subtree boundary prev<->B, reparents B/C/D to kept,
 * and rebuilds kept under new_parent_page_num. The external prev leaf keeps its
 * own parent. All caller-visible writes are withheld until the complete staged
 * image set validates.
 */
static inline bool tinydb_stage_root_height_inner_left_lower_merge(
    ...
    void* prev_leaf_page,
    size_t prev_leaf_capacity,
    uint32_t prev_leaf_page_num,
    uint32_t prev_leaf_parent_page_num) {
    if (obsolete_parent_page == NULL || kept_parent_page == NULL ||
        removed_leaf_page == NULL || survivor_leaf_pages == NULL ||
        survivor_leaf_page_nums == NULL || prev_leaf_page == NULL ||
        obsolete_capacity < PAGE_SIZE || kept_capacity < PAGE_SIZE ||
        removed_capacity < PAGE_SIZE || prev_leaf_capacity < PAGE_SIZE ||
        ...
        prev_leaf_page_num == 0u || prev_leaf_page_num == INVALID_PAGE_NUM ||
        prev_leaf_parent_page_num == 0u ||
        prev_leaf_parent_page_num == INVALID_PAGE_NUM ||
        obsolete_parent_page_num == kept_parent_page_num ||
        removed_leaf_page_num == prev_leaf_page_num ||
    ...
    const unsigned char* prev_leaf = (const unsigned char*)prev_leaf_page;
    if (!tinydb_parent_stage_validate(obsolete, obsolete_capacity) ||
        ...
        tinydb_leaf_format_detect_page(prev_leaf, prev_leaf_capacity) !=
            TINYDB_LEAF_PAGE_FORMAT_SLOTTED_V2 ||
        !tinydb_slotted_leaf_v2_validate(prev_leaf, prev_leaf_capacity)) {
        return false;
    }
    ...
    uint32_t prev_min = 0u, prev_max = 0u;
    uint32_t prev_prev = INVALID_PAGE_NUM, prev_next = INVALID_PAGE_NUM;
    if (!tinydb_nonroot_merge_leaf_valid(removed, removed_capacity,
        ...
        !tinydb_nonroot_merge_leaf_valid(prev_leaf, prev_leaf_capacity,
                                         prev_leaf_parent_page_num,
                                         &prev_min, &prev_max,
                                         &prev_prev, &prev_next) ||
        removed_min != removed_key || removed_max != removed_key ||
        removed_prev != prev_leaf_page_num ||
        removed_next != survivor_leaf_page_nums[0] ||
        prev_next != removed_leaf_page_num ||
        ...
        maxes[1] != tinydb_parent_stage_key_at(kept, 0u) ||
        prev_max >= removed_key || removed_key >= mins[0]) {
        return false;
    }
    (void)prev_min;
    (void)prev_prev;
    ...
    unsigned char prev_scratch[PAGE_SIZE];
    memcpy(kept_scratch, kept, PAGE_SIZE);
    memcpy(prev_scratch, prev_leaf, PAGE_SIZE);
    for (uint32_t i = 0u; i < 3u; i++) {
        memcpy(leaf_scratch[i], survivor_leaf_pages[i], PAGE_SIZE);
    }

    if (!tinydb_stage_leaf_sibling_relink(
            prev_scratch, PAGE_SIZE, true,
            removed_leaf_page_num, survivor_leaf_page_nums[0]) ||
        !tinydb_stage_leaf_sibling_relink(
            leaf_scratch[0], PAGE_SIZE, false,
            removed_leaf_page_num, prev_leaf_page_num)) {
        return false;
    }
    for (uint32_t i = 0u; i < 3u; i++) {
    ...
    uint32_t checked_prev_next = INVALID_PAGE_NUM;
    uint32_t first_prev = INVALID_PAGE_NUM;
    uint32_t checked_prev_parent = tinydb_parent_stage_read_u32(
        prev_scratch + PARENT_POINTER_OFFSET);
    if (!tinydb_leaf_page_next(prev_scratch, PAGE_SIZE, &checked_prev_next) ||
        !tinydb_leaf_page_prev(leaf_scratch[0], PAGE_SIZE, &first_prev) ||
        checked_prev_next != survivor_leaf_page_nums[0] ||
        first_prev != prev_leaf_page_num ||
        checked_prev_parent != prev_leaf_parent_page_num ||
        !tinydb_slotted_leaf_v2_validate(prev_scratch, PAGE_SIZE)) {
        return false;
    }
    ...
    memcpy(prev_leaf_page, prev_scratch, PAGE_USABLE_SIZE);
    return true;
}

#endif /* TINYDB_INTERNAL_ROOT_HEIGHT_INNER_LEFT_LOWER_STAGE_H */
