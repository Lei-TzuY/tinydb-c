...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="tinydb-v2-recursive-underflow-") as temp_dir:
        ...
        with open(os.path.join(temp_dir, "CMakeLists.txt"), "w", encoding="utf-8") as handle:
            handle.write(
                ...
                "project(tinydb_v2_recursive_internal_underflow_guard C)\n"
        ...
        if executable is None:
            raise AssertionError("recursive internal underflow guard probe was not produced")

        db_path = os.path.join(temp_dir, "v2_recursive_internal_underflow_guard.db")
        cleanup(db_path)
        ...
        for marker in (
            ...
            "unsupported_right_inner_right=yes",
            "height4=yes",
            "fail_closed=yes",
            "root_stable=yes",
            "ancestor_stable=yes",
    ...
    print(
        "PASS: the remaining key-60 right-inner-right height-4 V2 DELETE shape "
        "stays fail-closed with records, topology, links, allocator state, and "
        "reopen integrity unchanged while key 50 has a dedicated live route"
    )
