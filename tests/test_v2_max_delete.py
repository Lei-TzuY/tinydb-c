...
def main():
    ...
    source = os.path.join(repo_root, "tests", "v2_max_delete_probe.c")
    ...
    with tempfile.TemporaryDirectory(prefix="tinydb-v2-max-delete-") as temp_dir:
        ...
        with open(os.path.join(temp_dir, "CMakeLists.txt"), "w", encoding="utf-8") as handle:
            handle.write(
                ...
                "project(tinydb_v2_max_delete_probe C)\n"
                ...
                f'add_executable(v2_max_delete_probe "{source_cmake}")\n'
                f'target_include_directories(v2_max_delete_probe PRIVATE "{include_cmake}")\n'
                "target_link_libraries(v2_max_delete_probe PRIVATE tinydb_core)\n"
            )
        ...
        candidates = [
            os.path.join(build_dir, "Debug", "v2_max_delete_probe.exe"),
            os.path.join(build_dir, "Release", "v2_max_delete_probe.exe"),
            os.path.join(build_dir, "v2_max_delete_probe.exe"),
            os.path.join(build_dir, "v2_max_delete_probe"),
        ]
        executable = next((path for path in candidates if os.path.exists(path)), None)
        if executable is None:
            raise AssertionError("V2 max-delete live probe executable was not produced")

        db_path = os.path.join(temp_dir, "v2_max_delete.db")
        cleanup(db_path)
