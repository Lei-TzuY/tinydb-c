...
def main():
    ...
    source = os.path.join(repo_root, "tests", "internal_root_height_mirror_lower_stage_probe.c")
    ...
    with tempfile.TemporaryDirectory(prefix="tinydb-root-height-mirror-lower-") as temp_dir:
        ...
        with open(os.path.join(temp_dir, "CMakeLists.txt"), "w", encoding="utf-8") as handle:
            handle.write(
                ...
                "project(tinydb_root_height_mirror_lower C)\n"
                ...
                f'add_executable(internal_root_height_mirror_lower_stage_probe "{source_cmake}")\n'
                f'target_include_directories(internal_root_height_mirror_lower_stage_probe PRIVATE "{include_cmake}")\n'
                "target_link_libraries(internal_root_height_mirror_lower_stage_probe PRIVATE tinydb_core)\n"
            )
        ...
        candidates = [
            os.path.join(build_dir, "Debug", "internal_root_height_mirror_lower_stage_probe.exe"),
            os.path.join(build_dir, "Release", "internal_root_height_mirror_lower_stage_probe.exe"),
            os.path.join(build_dir, "internal_root_height_mirror_lower_stage_probe.exe"),
            os.path.join(build_dir, "internal_root_height_mirror_lower_stage_probe"),
        ]
        executable = next((path for path in candidates if os.path.exists(path)), None)
        if executable is None:
            raise AssertionError("mirrored lower merge staging probe was not produced")
        ...
        for marker in (
            "INTERNAL_ROOT_HEIGHT_MIRROR_LOWER_STAGE_OK",
            "lower_merge=yes",
    ...
    print(
        "PASS: mirrored recursive root-height lower merge stages [A, removed] + "
        "[C, D] into [A, C, D], repairs links/parents, preserves source and "
        "checksum bytes, and fails atomically"
    )
