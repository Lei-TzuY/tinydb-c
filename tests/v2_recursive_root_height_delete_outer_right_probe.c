...
static bool contracted_outer_right_state(
    Table* table,
    TableSchema* schema,
    ...
    const uint32_t kept_children[3] = {leaves[4], leaves[5], leaves[6]};
    ...
    if (!internal_matches(table, schema->root_page_num,
        ...
        !internal_matches(table, parents[2], kept_children, kept_keys, 3u) ||
    ...
    const uint32_t survivor_parents[7] = {
        ...
        parents[2], parents[2], parents[2]
    };
    for (uint32_t i = 0u; i < 7u; i++) {
        uint32_t prev = i == 0u ? 0u : leaves[i - 1u];
        uint32_t next = i + 1u == 7u ? 0u : leaves[i + 1u];
        if (!leaf_state(table, leaves[i], survivor_parents[i], prev, next) ||
...
static bool run_outer_right_case(const char* path) {
    remove(path);
    ...
    if (!exec_ok(db, "BEGIN;") ||
        ...
        !contracted_outer_right_state(table, schema, parents, leaves, free_before) ||
        !exec_ok(db, "PRAGMA integrity_check;") ||
        !exec_ok(db, "ROLLBACK;") ||
        !unchanged_state(table, schema, grands, parents, leaves, free_before) ||
        !exec_ok(db, "PRAGMA integrity_check;")) {
        fprintf(stderr, "outer-right root-height rollback failed: %s\n", message);
        tinydb_close(db);
        return false;
    }
    ...
    if (!tinydb_record_delete(table, schema, 80u, message, sizeof(message)) ||
        ...
        !contracted_outer_right_state(table, schema, parents, leaves, free_before) ||
        !exec_ok(db, "PRAGMA integrity_check;")) {
        fprintf(stderr, "outer-right root-height autocommit failed: %s\n", message);
        tinydb_close(db);
        return false;
    }
    tinydb_close(db);
    ...
    bool ok = schema != NULL && schema->root_page_num == root_before &&
              contracted_outer_right_state(table, schema, parents, leaves,
                                           free_before) &&
              exec_ok(db, "PRAGMA integrity_check;");
    tinydb_close(db);
    return ok;
...
int main(int argc, char** argv) {
    ...
    if (!run_outer_right_case(argv[1])) return EXIT_FAILURE;
    printf("V2_RECURSIVE_ROOT_HEIGHT_DELETE_OUTER_RIGHT_OK deleted=80 "
           "outer_right=yes height4_to_height3=yes root_identity=yes "
           "chain_tail=yes bottom_merge=yes grandparents_reclaimed=yes "
