...
static bool raw_insert(const TableSchema* schema,
    ...
    snprintf(values[1].text, sizeof(values[1].text), "wide-borrow-%u", id);
    values[2].type = COL_TYPE_INT;
    values[2].int_value = id + 5000u;
...
static bool allocate_tree_pages(Pager* pager,
...
static bool seed_case(TinyDB* db,
    ...
    const uint32_t keys[LEAF_COUNT] = {10u, 20u, 30u, 40u, 50u, 60u, 70u};
    if (!allocate_tree_pages(pager, parents, leaves)) return false;
    ...
    if (borrow_from_right) {
        ...
        const uint32_t p1_children[3] = {leaves[2], leaves[3], leaves[4]};
        const uint32_t p1_keys[2] = {30u, 40u};
        const uint32_t p2_children[2] = {leaves[5], leaves[6]};
        const uint32_t p2_keys[1] = {60u};
        const uint32_t root_children[3] = {parents[0], parents[1], parents[2]};
        const uint32_t root_keys[2] = {20u, 50u};
        if (!build_internal(pager, parents[0], root, false,
                            p0_children, p0_keys, 2u) ||
            !build_internal(pager, parents[1], root, false,
                            p1_children, p1_keys, 3u) ||
        ...
        const uint32_t p0_children[3] = {leaves[0], leaves[1], leaves[2]};
        const uint32_t p0_keys[2] = {10u, 20u};
        const uint32_t p1_children[2] = {leaves[3], leaves[4]};
        const uint32_t p1_keys[1] = {40u};
        const uint32_t p2_children[2] = {leaves[5], leaves[6]};
        const uint32_t p2_keys[1] = {60u};
        const uint32_t root_children[3] = {parents[0], parents[1], parents[2]};
        const uint32_t root_keys[2] = {30u, 50u};
        if (!build_internal(pager, parents[0], root, false,
                            p0_children, p0_keys, 3u) ||
            !build_internal(pager, parents[1], root, false,
...
static bool state_after_borrow(Table* table,
                               TableSchema* schema,
                               bool borrow_from_right,
    ...
    const uint32_t root_children[3] = {parents[0], parents[1], parents[2]};
    if (borrow_from_right) {
        const uint32_t p0_children[2] = {leaves[0], leaves[2]};
        const uint32_t p0_keys[1] = {10u};
        const uint32_t p1_children[2] = {leaves[3], leaves[4]};
        const uint32_t p1_keys[1] = {40u};
        const uint32_t p2_children[2] = {leaves[5], leaves[6]};
        ...
        return internal_matches(table, parents[0], p0_children, p0_keys, 2u) &&
               ...
               tinydb_record_scan(table, schema, NULL, NULL) == 6u;
    }

    const uint32_t p0_children[2] = {leaves[0], leaves[1]};
    const uint32_t p0_keys[1] = {10u};
    const uint32_t p1_children[2] = {leaves[2], leaves[4]};
...
static bool original_state(Table* table,
    ...
    if (borrow_from_right) {
        ...
        const uint32_t root_keys[2] = {20u, 50u};
        return internal_matches(table, parents[0], p0_children, p0_keys, 2u) &&
               internal_matches(table, parents[1], p1_children, p1_keys, 3u) &&
               internal_matches(table, parents[2], p2_children, p2_keys, 2u) &&
               internal_matches(table, schema->root_page_num,
                                root_children, root_keys, 3u) &&
    ...
    const uint32_t p2_keys[1] = {60u};
    const uint32_t root_keys[2] = {30u, 50u};
    return internal_matches(table, parents[0], p0_children, p0_keys, 3u) &&
           internal_matches(table, parents[1], p1_children, p1_keys, 2u) &&
           internal_matches(table, parents[2], p2_children, p2_keys, 2u) &&
           internal_matches(table, schema->root_page_num,
                            root_children, root_keys, 3u) &&
...
static bool run_case(const char* path, bool borrow_from_right) {
    remove(path);
    ...
    uint32_t leaves[LEAF_COUNT] = {0u, 0u, 0u, 0u, 0u, 0u, 0u};
    if (schema == NULL || schema->row_size != 264u ||
        !seed_case(db, schema, borrow_from_right, parents, leaves)) {
        fprintf(stderr, "unable to seed wide-root borrow tree\n");
        tinydb_close(db);
        return false;
    }
    uint32_t free_before = table->pager->free_page_count;
    uint32_t removed_key = borrow_from_right ? 20u : 40u;
    char message[TINYDB_RECORD_MESSAGE_MAX];

    if (!exec_ok(db, "BEGIN;") ||
        !delete_key(table, schema, removed_key, message) ||
        !state_after_borrow(table,
                            schema,
                            borrow_from_right,
                            parents,
                            leaves,
        ...
        !exec_ok(db, "PRAGMA integrity_check;")) {
        fprintf(stderr,
                "wide-root borrow rollback failed for key %u: %s\n",
                removed_key,
                message);
        tinydb_close(db);
        return false;
    ...
    if (!delete_key(table, schema, removed_key, message) ||
        ...
        !exec_ok(db, "PRAGMA integrity_check;")) {
        fprintf(stderr,
                "wide-root borrow commit failed for key %u: %s\n",
                removed_key,
                message);
        tinydb_close(db);
        return false;
    ...
    if (!ok) {
        fprintf(stderr,
                "wide-root borrow did not survive reopen for key %u\n",
                removed_key);
    }
    tinydb_close(db);
    return ok;
...
int main(int argc, char** argv) {
    ...
    printf("V2_INTERNAL_BORROW_WIDE_ROOT_OK right=yes left=yes wide_root=yes "
           "local_separator=yes donor_reparent=yes third_subtree=yes rollback=yes "
           "allocator=yes reopen=yes integrity=yes wal=yes\n");
    return EXIT_SUCCESS;
}
