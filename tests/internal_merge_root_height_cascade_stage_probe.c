...
typedef struct {
    ...
    unsigned char parents[4][PAGE_SIZE];
...
static bool seed(Fixture* f) {
    ...
    memset(f, 0, sizeof(*f));
    const uint32_t parent_pages[4] = {P0_PAGE,P1_PAGE,Q0_PAGE,Q1_PAGE};
    ...
    const uint32_t right_children[2] = {Q0_PAGE,Q1_PAGE};
...
static bool stage(Fixture* f) {
    ...
    return tinydb_stage_internal_merge_root_height_cascade(
        ...
        f->parents[3], PAGE_SIZE, Q1_PAGE,
        f->leaves[2], PAGE_SIZE, 32u, 30u,
        survivors, survivor_nums,
        f->leaves[4], PAGE_SIZE, 34u,
        f->leaves[7], PAGE_SIZE, 37u);
}
...
static bool test_success(void) {
    ...
    unsigned char q0_before[PAGE_SIZE];
    unsigned char q1_before[PAGE_SIZE];
    ...
    memcpy(q0_before, f.parents[2], PAGE_SIZE);
    memcpy(q1_before, f.parents[3], PAGE_SIZE);
    ...
    if (!stage(&f)) return false;

    const uint32_t root_children[3] = {P1_PAGE,Q0_PAGE,Q1_PAGE};
    const uint32_t root_keys[2] = {40u,60u};
    const uint32_t kept_children[3] = {30u,31u,33u};
    const uint32_t kept_keys[2] = {10u,20u};
    const uint32_t q0_children[2] = {34u,35u};
    const uint32_t q0_keys[1] = {50u};
    const uint32_t q1_children[2] = {36u,37u};
    const uint32_t q1_keys[1] = {70u};

    return internal_matches(f.root,0u,true,root_children,root_keys,3u) &&
           internal_matches(f.parents[1],ROOT_PAGE,false,
                            kept_children,kept_keys,3u) &&
           internal_matches(f.parents[2],ROOT_PAGE,false,
                            q0_children,q0_keys,2u) &&
           internal_matches(f.parents[3],ROOT_PAGE,false,
           ...
           trailer_same(f.parents[2],q0_before) &&
           trailer_same(f.parents[3],q1_before) &&
...
static bool test_atomic_failure(void) {
    ...
    return !ok &&
           ...
           memcmp(f.parents[2],q0_before,PAGE_SIZE) == 0 &&
           memcmp(f.parents[3],q1_before,PAGE_SIZE) == 0 &&
...
int main(void) {
    ...
    printf("INTERNAL_MERGE_ROOT_HEIGHT_CASCADE_STAGE_OK bottom_merge=yes "
           "grandparent_merge=yes root_collapse=yes height4_to_height3=yes "
           "root_identity=yes source_immutable=yes trailer=yes "
           "atomic_failure=yes\n");
    return EXIT_SUCCESS;
}
