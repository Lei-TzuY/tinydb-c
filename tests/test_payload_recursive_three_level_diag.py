...
def main():
    ...
    source = os.path.join(repo_root, "tests", "payload_recursive_three_level_diag.c")
    ...
    with tempfile.TemporaryDirectory(prefix="tinydb-three-level-diag-") as temp_dir:
        ...
        with open(os.path.join(temp_dir, "CMakeLists.txt"), "w", encoding="utf-8") as handle:
            handle.write(
                ...
                "project(tinydb_three_level_diag C)\n"
                ...
                f'add_executable(payload_recursive_three_level_diag "{source_cmake}")\n'
                f'target_include_directories(payload_recursive_three_level_diag PRIVATE "{include_cmake}" "{tests_cmake}")\n'
                "target_link_libraries(payload_recursive_three_level_diag PRIVATE tinydb_core)\n"
            )
        ...
        candidates = [
            os.path.join(build_dir, "Debug", "payload_recursive_three_level_diag.exe"),
            os.path.join(build_dir, "Release", "payload_recursive_three_level_diag.exe"),
            os.path.join(build_dir, "payload_recursive_three_level_diag.exe"),
            os.path.join(build_dir, "payload_recursive_three_level_diag"),
        ]
        executable = next((path for path in candidates if os.path.exists(path)), None)
        if executable is None:
            raise AssertionError("three-level diagnostic executable was not produced")

        db_path = os.path.join(temp_dir, "diag.db")
