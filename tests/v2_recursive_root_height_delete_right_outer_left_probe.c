...
static bool contracted_right_outer_left_state(
    Table* table,
    TableSchema* schema,
    ...
    const uint32_t survivor_indexes[7] = {0u, 1u, 2u, 3u, 4u, 5u, 7u};
...
static bool run_right_outer_left_case(const char* path) {
    remove(path);
    ...
    if (!exec_ok(db, "BEGIN;") ||
        ...
        !contracted_right_outer_left_state(table, schema, parents, leaves,
                                           free_before) ||
        !exec_ok(db, "PRAGMA integrity_check;") ||
        !exec_ok(db, "ROLLBACK;") ||
        !unchanged_state(table, schema, grands, parents, leaves, free_before) ||
        !exec_ok(db, "PRAGMA integrity_check;")) {
        fprintf(stderr, "right-outer-left root-height rollback failed: %s\n", message);
        tinydb_close(db);
        return false;
    }
    ...
    if (!tinydb_record_delete(table, schema, 70u, message, sizeof(message)) ||
        ...
        !contracted_right_outer_left_state(table, schema, parents, leaves,
                                           free_before) ||
        !exec_ok(db, "PRAGMA integrity_check;")) {
        fprintf(stderr, "right-outer-left root-height autocommit failed: %s\n", message);
        tinydb_close(db);
        return false;
    }
    tinydb_close(db);
...
int main(int argc, char** argv) {
    ...
    if (!run_right_outer_left_case(argv[1])) return EXIT_FAILURE;
    printf("V2_RECURSIVE_ROOT_HEIGHT_DELETE_RIGHT_OUTER_LEFT_OK deleted=70 "
           "right_outer_left=yes height4_to_height3=yes root_identity=yes "
           "shared_boundary_relink=yes bottom_merge=yes grandparents_reclaimed=yes "
