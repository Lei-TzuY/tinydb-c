...
static bool contracted_inner_right_state(
    Table* table,
    TableSchema* schema,
    const uint32_t parents[LEAF_PARENT_COUNT],
    const uint32_t leaves[LEAF_COUNT],
    uint32_t free_before) {
    const uint32_t root_children[3] = {parents[0], parents[2], parents[3]};
    const uint32_t root_keys[2] = {30u, 60u};
    const uint32_t kept_children[3] = {leaves[0], leaves[1], leaves[2]};
    ...
    if (!internal_matches(table, schema->root_page_num,
        ...
        !internal_matches(table, parents[0], kept_children, kept_keys, 3u) ||
    ...
    const uint32_t survivor_indexes[7] = {0u, 1u, 2u, 4u, 5u, 6u, 7u};
    const uint32_t survivor_parents[7] = {
        parents[0], parents[0], parents[0],
...
static bool run_inner_right_case(const char* path) {
    remove(path);
    ...
    if (!exec_ok(db, "BEGIN;") ||
        !tinydb_record_delete(table, schema, 40u, message, sizeof(message)) ||
        schema->root_page_num != root_before ||
        !contracted_inner_right_state(table, schema, parents, leaves, free_before) ||
        !exec_ok(db, "PRAGMA integrity_check;") ||
        !exec_ok(db, "ROLLBACK;") ||
        !unchanged_state(table, schema, grands, parents, leaves, free_before) ||
        !exec_ok(db, "PRAGMA integrity_check;")) {
        fprintf(stderr, "inner-right root-height rollback failed: %s\n", message);
        tinydb_close(db);
        return false;
    }

    message[0] = '\0';
    if (!tinydb_record_delete(table, schema, 40u, message, sizeof(message)) ||
        schema->root_page_num != root_before ||
        !contracted_inner_right_state(table, schema, parents, leaves, free_before) ||
        !exec_ok(db, "PRAGMA integrity_check;")) {
        fprintf(stderr, "inner-right root-height autocommit failed: %s\n", message);
        tinydb_close(db);
        return false;
    }
    tinydb_close(db);
    ...
    bool ok = schema != NULL && schema->root_page_num == root_before &&
              contracted_inner_right_state(table, schema, parents, leaves,
                                           free_before) &&
              exec_ok(db, "PRAGMA integrity_check;");
    tinydb_close(db);
    return ok;
...
int main(int argc, char** argv) {
    ...
    if (!run_inner_right_case(argv[1])) return EXIT_FAILURE;
    printf("V2_RECURSIVE_ROOT_HEIGHT_DELETE_INNER_RIGHT_OK deleted=40 "
           "inner_right=yes height4_to_height3=yes root_identity=yes "
           "root_max_decrease=yes cross_boundary_relink=yes "
           "bottom_merge=yes grandparents_reclaimed=yes rollback=yes "
           "allocator=yes wal=yes reopen=yes integrity=yes\n");
    return EXIT_SUCCESS;
}
