...
static bool contracted_outer_left_state(
    Table* table,
    TableSchema* schema,
    ...
    const uint32_t kept_children[3] = {leaves[1], leaves[2], leaves[3]};
    const uint32_t kept_keys[2] = {20u, 30u};
    ...
    if (!internal_matches(table, schema->root_page_num,
        ...
        present(table, schema, 10u)) {
        return false;
    }

    const uint32_t survivor_indexes[7] = {1u, 2u, 3u, 4u, 5u, 6u, 7u};
...
static bool run_outer_left_case(const char* path) {
    remove(path);
    ...
    if (!exec_ok(db, "BEGIN;") ||
        !tinydb_record_delete(table, schema, 10u, message, sizeof(message)) ||
        schema->root_page_num != root_before ||
        !contracted_outer_left_state(table, schema, parents, leaves, free_before) ||
        !exec_ok(db, "PRAGMA integrity_check;") ||
        !exec_ok(db, "ROLLBACK;") ||
        !unchanged_state(table, schema, grands, parents, leaves, free_before) ||
        !exec_ok(db, "PRAGMA integrity_check;")) {
        fprintf(stderr, "outer-left root-height rollback failed: %s\n", message);
        tinydb_close(db);
        return false;
    }

    message[0] = '\0';
    if (!tinydb_record_delete(table, schema, 10u, message, sizeof(message)) ||
        schema->root_page_num != root_before ||
        !contracted_outer_left_state(table, schema, parents, leaves, free_before) ||
        !exec_ok(db, "PRAGMA integrity_check;")) {
        fprintf(stderr, "outer-left root-height autocommit failed: %s\n", message);
        tinydb_close(db);
        return false;
    }
    tinydb_close(db);
    ...
    bool ok = schema != NULL && schema->root_page_num == root_before &&
              contracted_outer_left_state(table, schema, parents, leaves,
                                          free_before) &&
              exec_ok(db, "PRAGMA integrity_check;");
    tinydb_close(db);
    return ok;
...
int main(int argc, char** argv) {
    ...
    if (!run_outer_left_case(argv[1])) return EXIT_FAILURE;
    printf("V2_RECURSIVE_ROOT_HEIGHT_DELETE_OUTER_LEFT_OK deleted=10 "
           "outer_left=yes height4_to_height3=yes root_identity=yes "
           "chain_head=yes bottom_merge=yes grandparents_reclaimed=yes "
