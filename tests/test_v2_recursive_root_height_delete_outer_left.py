...
def main():
    ...
    source = os.path.join(repo_root, "tests", "v2_recursive_root_height_delete_outer_left_probe.c")
    ...
    with tempfile.TemporaryDirectory(prefix="tinydb-v2-root-height-outer-left-") as temp_dir:
        ...
        with open(os.path.join(temp_dir, "CMakeLists.txt"), "w", encoding="utf-8") as handle:
            handle.write(
                ...
                "project(tinydb_v2_root_height_outer_left C)\n"
                ...
                f'add_executable(v2_recursive_root_height_delete_outer_left_probe "{source_cmake}")\n'
                f'target_include_directories(v2_recursive_root_height_delete_outer_left_probe PRIVATE "{include_cmake}" "{tests_cmake}")\n'
                "target_link_libraries(v2_recursive_root_height_delete_outer_left_probe PRIVATE tinydb_core)\n"
            )
        ...
        candidates = [
            os.path.join(build_dir, "Debug", "v2_recursive_root_height_delete_outer_left_probe.exe"),
            os.path.join(build_dir, "Release", "v2_recursive_root_height_delete_outer_left_probe.exe"),
            os.path.join(build_dir, "v2_recursive_root_height_delete_outer_left_probe.exe"),
            os.path.join(build_dir, "v2_recursive_root_height_delete_outer_left_probe"),
        ]
        executable = next((path for path in candidates if os.path.exists(path)), None)
        if executable is None:
            raise AssertionError("outer-left recursive root-height probe was not produced")
        db_path = os.path.join(temp_dir, "outer-left.db")
        cleanup(db_path)
        ...
        for marker in (
            "V2_RECURSIVE_ROOT_HEIGHT_DELETE_OUTER_LEFT_OK",
            "deleted=10",
            "outer_left=yes",
    ...
    print(
        "PASS: outer-left height-4 V2 DELETE removes key 10, repairs the leaf "
        "chain head, contracts the stable root to height three, restores the "
