...
static bool contracted_state(Table* table,
    ...
    const uint32_t survivor_indexes[7] = {0u, 1u, 3u, 4u, 5u, 6u, 7u};
...
static bool run_root_height_case(const char* path) {
    remove(path);
    ...
    if (!tinydb_record_delete(table,
        ...
        !contracted_state(table, schema, parents, leaves, free_before) ||
        !exec_ok(db, "PRAGMA integrity_check;")) {
        fprintf(stderr, "recursive root-height delete failed: %s\n", message);
        tinydb_close(db);
        return false;
    }
...
int main(int argc, char** argv) {
    ...
    if (!run_root_height_case(argv[1])) return EXIT_FAILURE;
    printf("V2_RECURSIVE_ROOT_HEIGHT_DELETE_OK deleted=30 height4_to_height3=yes "
           "root_identity=yes bottom_merge=yes grandparents_reclaimed=yes "
