...
def main():
    ...
    source = os.path.join(repo_root, "tests", "v2_empty_leaf_boundary_probe.c")
    ...
    with tempfile.TemporaryDirectory(prefix="tinydb-v2-empty-boundary-") as temp_dir:
        ...
        with open(os.path.join(temp_dir, "CMakeLists.txt"), "w", encoding="utf-8") as handle:
            handle.write(
                ...
                "project(tinydb_v2_empty_leaf_boundary_probe C)\n"
                ...
                f'add_executable(v2_empty_boundary_probe "{source_cmake}")\n'
                f'target_include_directories(v2_empty_boundary_probe PRIVATE "{include_cmake}")\n'
                "target_link_libraries(v2_empty_boundary_probe PRIVATE tinydb_core)\n"
            )
        ...
        candidates = [
            os.path.join(build_dir, "Debug", "v2_empty_boundary_probe.exe"),
            os.path.join(build_dir, "Release", "v2_empty_boundary_probe.exe"),
            os.path.join(build_dir, "v2_empty_boundary_probe.exe"),
            os.path.join(build_dir, "v2_empty_boundary_probe"),
        ]
        executable = next((path for path in candidates if os.path.exists(path)), None)
        if executable is None:
            raise AssertionError("V2 boundary empty-leaf probe executable was not produced")

        db_path = os.path.join(temp_dir, "v2_empty_boundary.db")
        cleanup(db_path)
