 ...
 * on an orientation that remains unsupported. Deletes 10, 20, 30, and 80 now
 * have explicit height-contraction routes; deleting key 40 (the rightmost leaf
 * of the right bottom parent under the left grandparent) requires a different
 * lower-merge orientation and must still fail without mutating topology or
 * allocator state.
 */
...
int main(int argc, char** argv) {
    ...
    if (!run_case(path, 40u)) return EXIT_FAILURE;
    printf("V2_RECURSIVE_INTERNAL_UNDERFLOW_GUARD_OK "
           "unsupported_inner_right=yes height4=yes fail_closed=yes "
