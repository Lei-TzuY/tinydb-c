 ...
 * on an orientation that remains unsupported. Deletes 10, 20, and 30 now have
 * explicit height-contraction routes; deleting the outer-right key 80 requires
 * the symmetric chain-tail lower merge and must still fail without mutating
 * topology or allocator state.
 */
...
int main(int argc, char** argv) {
    ...
    if (!run_case(path, 80u)) return EXIT_FAILURE;
    printf("V2_RECURSIVE_INTERNAL_UNDERFLOW_GUARD_OK "
           "unsupported_outer_right=yes height4=yes fail_closed=yes "
