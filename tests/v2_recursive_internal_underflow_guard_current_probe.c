 ...
 * on an orientation that remains unsupported. Deletes 10, 20, 30, 40, and 80
 * now have explicit height-contraction routes; deleting key 50 (the left leaf
 * of the left bottom parent under the right grandparent) requires the remaining
 * mirrored cross-grand lower-merge orientation and must still fail without
...
int main(int argc, char** argv) {
    ...
    if (!run_case(path, 50u)) return EXIT_FAILURE;
    printf("V2_RECURSIVE_INTERNAL_UNDERFLOW_GUARD_OK "
           "unsupported_right_inner_left=yes height4=yes fail_closed=yes "
