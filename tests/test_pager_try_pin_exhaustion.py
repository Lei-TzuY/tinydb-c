...
def main():
    ...
    probe_source = os.path.join(repo_root, "tests", "pager_try_pin_exhaustion_probe.c")
    ...
    with tempfile.TemporaryDirectory(prefix="tinydb-try-pin-exhaustion-") as temp_dir:
        ...
        with open(cmake_lists, "w", encoding="utf-8") as handle:
            handle.write(
                ...
                "project(tinydb_pager_try_pin_exhaustion_probe C)\n"
                ...
                f'add_executable(pager_try_pin_exhaustion_probe "{paths["probe"]}" '
                ...
                f'target_include_directories(pager_try_pin_exhaustion_probe PRIVATE "{paths["src"]}")\n'
                "target_link_libraries(pager_try_pin_exhaustion_probe PRIVATE Threads::Threads)\n"
            )
        ...
        candidates = [
            os.path.join(build_dir, "Debug", "pager_try_pin_exhaustion_probe.exe"),
            os.path.join(build_dir, "Release", "pager_try_pin_exhaustion_probe.exe"),
            os.path.join(build_dir, "pager_try_pin_exhaustion_probe.exe"),
            os.path.join(build_dir, "pager_try_pin_exhaustion_probe"),
        ]
        executable = next((path for path in candidates if os.path.exists(path)), None)
        if executable is None:
            raise AssertionError("try-pin exhaustion probe executable was not produced")

        db_path = os.path.join(temp_dir, "try-pin-exhaustion.db")
