...
int main(void) {
    ...
    /* A merely materialized/not-durable tree may never publish recovery intent. */
    durable.ready = false;
    memset(&probe, 0, sizeof(probe));
    ops = make_ops(&probe);
    if (tinydb_schema_repack_publish_durable_migration_intent(
    ...
    /* Claimed-page identity must exactly match the durable staging result. */
    memset(&probe, 0, sizeof(probe));
    ops = make_ops(&probe);
    if (tinydb_schema_repack_publish_durable_migration_intent(
    ...
    /* The manifest validator rejects an old root inside the staging namespace. */
    {
    ...
    /* A failure at the final durability boundary must not publish result.ready. */
    memset(&probe, 0, sizeof(probe));
