...
static bool raw_insert(const TableSchema* schema,
    ...
    snprintf(values[1].text, sizeof(values[1].text), "recursive-cascade-%u", id);
    values[2].type = COL_TYPE_INT;
    values[2].int_value = id + 12000u;
...
static bool seed_tree(TinyDB* db,
    ...
    const uint32_t right_children[3] = {parents[2],parents[3],parents[4]};
...
static bool cascaded_state(Table* table,
    ...
    if (!internal_matches(table, schema->root_page_num, 0u, true,
        ...
        tinydb_record_scan(table, schema, NULL, NULL) != 9u) return false;

    const uint32_t q1_children[2] = {leaves[6],leaves[7]};
    const uint32_t q1_keys[1] = {70u};
    const uint32_t q2_children[2] = {leaves[8],leaves[9]};
    const uint32_t q2_keys[1] = {90u};
    return internal_matches(table, parents[3], grands[1], false,
                            q1_children, q1_keys, 2u) &&
           internal_matches(table, parents[4], grands[1], false,
                            q2_children, q2_keys, 2u) &&
...
static bool run_case(const char* path) {
    ...
    char message[TINYDB_RECORD_MESSAGE_MAX];
    if (!exec_ok(db, "BEGIN;") ||
        !(message[0] = '\0', tinydb_record_delete(table, schema, 30u,
                                                   message, sizeof(message))) ||
        ...
        !exec_ok(db, "PRAGMA integrity_check;")) {
        fprintf(stderr, "recursive cascade rollback failed: %s\n", message);
        tinydb_close(db);
        return false;
    }
    ...
    if (!tinydb_record_delete(table, schema, 30u, message, sizeof(message)) ||
        !cascaded_state(table, schema, grands, parents, leaves, free_before) ||
        !exec_ok(db, "PRAGMA integrity_check;")) {
        fprintf(stderr, "recursive cascade commit failed: %s\n", message);
        tinydb_close(db);
        return false;
    }
...
int main(int argc, char** argv) {
    ...
    printf("V2_RECURSIVE_INTERNAL_MERGE_BORROW_OK bottom_merge=yes "
           "grandparent_underflow=yes upper_borrow=yes root_separator=yes "
           "height4=yes height_preserved=yes rollback=yes allocator=yes "
           "reopen=yes integrity=yes wal=yes fail_closed_guard_preserved=yes\n");
    return EXIT_SUCCESS;
}
