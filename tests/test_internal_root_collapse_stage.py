...
def main():
    ...
    source = os.path.join(repo_root, "tests", "internal_root_collapse_stage_probe.c")
    core_library = find_core_library(repo_root)
    if not os.path.exists(source):
        raise AssertionError("missing internal_root_collapse_stage_probe.c")
    if core_library is None:
        raise AssertionError("could not find built tinydb_core static library")

    with tempfile.TemporaryDirectory(prefix="tinydb-root-collapse-stage-") as temp_dir:
        ...
        with open(os.path.join(temp_dir, "CMakeLists.txt"), "w", encoding="utf-8") as handle:
            handle.write(
                ...
                "project(tinydb_internal_root_collapse_stage C)\n"
                ...
                f'add_executable(root_collapse_stage_probe "{source_cmake}")\n'
                f'target_include_directories(root_collapse_stage_probe PRIVATE "{include_cmake}")\n'
                "target_link_libraries(root_collapse_stage_probe PRIVATE tinydb_core)\n"
            )
        ...
        candidates = [
            os.path.join(build_dir, "Debug", "root_collapse_stage_probe.exe"),
            os.path.join(build_dir, "Release", "root_collapse_stage_probe.exe"),
            os.path.join(build_dir, "root_collapse_stage_probe.exe"),
            os.path.join(build_dir, "root_collapse_stage_probe"),
        ]
        executable = next((path for path in candidates if os.path.exists(path)), None)
        if executable is None:
            raise AssertionError("root-collapse stage probe executable was not produced")
        ...
        for marker in (
            "INTERNAL_ROOT_COLLAPSE_STAGE_OK",
            "left=yes",
            "right=yes",
            "v2_root=yes",
