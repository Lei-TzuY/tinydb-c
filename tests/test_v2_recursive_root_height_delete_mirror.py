...
def main():
    ...
    source = os.path.join(repo_root, "tests", "v2_recursive_root_height_delete_mirror_probe.c")
    ...
    with tempfile.TemporaryDirectory(prefix="tinydb-v2-root-height-mirror-") as temp_dir:
        ...
        with open(os.path.join(temp_dir, "CMakeLists.txt"), "w", encoding="utf-8") as handle:
            handle.write(
                ...
                "project(tinydb_v2_root_height_mirror C)\n"
                ...
                f'add_executable(v2_recursive_root_height_delete_mirror_probe "{source_cmake}")\n'
                f'target_include_directories(v2_recursive_root_height_delete_mirror_probe PRIVATE "{include_cmake}" "{tests_cmake}")\n'
                "target_link_libraries(v2_recursive_root_height_delete_mirror_probe PRIVATE tinydb_core)\n"
            )
        ...
        candidates = [
            os.path.join(build_dir, "Debug", "v2_recursive_root_height_delete_mirror_probe.exe"),
            os.path.join(build_dir, "Release", "v2_recursive_root_height_delete_mirror_probe.exe"),
            os.path.join(build_dir, "v2_recursive_root_height_delete_mirror_probe.exe"),
            os.path.join(build_dir, "v2_recursive_root_height_delete_mirror_probe"),
        ]
        executable = next((path for path in candidates if os.path.exists(path)), None)
        if executable is None:
            raise AssertionError("mirrored recursive root-height probe was not produced")
        ...
        for marker in (
            "V2_RECURSIVE_ROOT_HEIGHT_DELETE_MIRROR_OK",
            "deleted=20",
            "mirror=yes",
    ...
    print(
        "PASS: mirrored height-4 V2 DELETE removes key 20, contracts the stable "
        "root to height three, restores topology/free-list on rollback, and "
        "persists cleanly through WAL/reopen"
    )
