...
static bool seed(Fixture* f) {
    ...
    memset(f, 0, sizeof(*f));
    const uint32_t parent_pages[5] = {P0_PAGE,P1_PAGE,P2_PAGE,Q0_PAGE,Q1_PAGE};
    ...
    for (uint32_t i = 0u; i < 5u; i++) {
        ...
        uint32_t grand = i < 3u ? LEFT_GRAND_PAGE : RIGHT_GRAND_PAGE;
    ...
    const uint32_t left_children[3] = {P0_PAGE,P1_PAGE,P2_PAGE};
...
static bool test_atomic_failure(void) {
    ...
    const void* donor_leaves[2] = {f.leaves[4],f.leaves[5]};
    const uint32_t donor_nums[2] = {34u,35u};
    bool ok = tinydb_stage_internal_merge_borrow_from_left_grandparent(
        f.root, PAGE_SIZE, ROOT_PAGE,
        f.left_grand, PAGE_SIZE, LEFT_GRAND_PAGE,
        f.right_grand, PAGE_SIZE, RIGHT_GRAND_PAGE,
        f.parents[2], PAGE_SIZE, P2_PAGE,
...
int main(void) {
    ...
    printf("INTERNAL_MERGE_BORROW_ROOT_CASCADE_LEFT_STAGE_OK "
           "mirror=yes root_separator_down=yes donor_subtree=yes "
           "lower_merge=yes reparent=yes source_immutable=yes "
           "checksum=yes atomic_failure=yes\n");
    return EXIT_SUCCESS;
}
