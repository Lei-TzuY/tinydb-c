...
/* Minimal deterministic Pager backing for the transaction-scoped composition
 * seam. The production Pager implementation has independent regressions; this
 * probe focuses on exact identity claiming, leaf-namespace rebinding,
 * hierarchy construction, dirty-page materialization, and root publication. */
...
int main(void) {
    ...
    pager.num_pages = 1u; /* page zero remains reserved */
    ...
    uint32_t original_leaf_count = leaves.page_count;
    ...
    if (!result.ready || result.row_count != 120u ||
        result.leaf_page_count != original_leaf_count ||
        ...
        result.root_page_num == 0u ||
        leaves.page_numbers != claimed_pages ||
        !tinydb_compact_v2_staging_hierarchy_validate(&hierarchy) ||
        !tinydb_compact_v2_staging_pager_verify_materialized_hierarchy(
            &pager, &hierarchy, claimed_pages, result.claimed_page_count)) return 5;

    for (uint32_t i = 0u; i < result.claimed_page_count; i++) {
        if (claimed_pages[i] != i + 1u || !dirty[claimed_pages[i]]) return 6;
    }
    for (uint32_t i = 0u; i < leaves.page_count; i++) {
        ...
        if (memcmp(actual, expected, PAGE_SIZE) != 0) return 7;
    }

    /* Invalid rebinding must fail before changing an already valid namespace. */
    uint32_t duplicate_pages[16];
    for (uint32_t i = 0u; i < leaves.page_count; i++) duplicate_pages[i] = claimed_pages[i];
    if (leaves.page_count > 1u) duplicate_pages[1] = duplicate_pages[0];
    if (tinydb_schema_repack_staging_rebind_leaf_pages(
            &leaves, duplicate_pages, leaves.page_count) ||
        leaves.page_numbers != claimed_pages ||
        !tinydb_compact_v2_staging_leaf_chain_validate(&leaves)) return 8;

    puts("SCHEMA_REPACK_STAGING_PAGER_OK");
    return 0;
}
'''
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if "SCHEMA_REPACK_STAGING_PAGER_OK" not in run.stdout:
